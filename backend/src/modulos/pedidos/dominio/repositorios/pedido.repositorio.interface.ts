...
// Porta (Port) — define o contrato sem depender de infraestrutura
export interface IPedidoRepositorio {
  ...
  contarPorStatus(): Promise<{ status: StatusPedido; quantidade: number }[]>;
  somarFaturamentoPorDia(dias: number): Promise<{ data: string; valor: number }[]>;
}
